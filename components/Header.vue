<script setup>
	const typeObj = reactive({
		typeValue: "",
		typeStatus: false,
		typeArray: [
			"Welcome!",
			"Hola!",
			"你好!",
			"Bonjour!",
			"Hello!",
			"Ciao!",
			"こんにちは!",
			"안녕하세요!",
		],
		typingSpeed: 75,
		erasingSpeed: 75,
		newTextDelay: 1000,
		typeArrayIndex: 0,
		charIndex: 0,
	});

	function typeText() {
		if (typeObj.charIndex < typeObj.typeArray[typeObj.typeArrayIndex].length) {
			if (!typeObj.typeStatus) {
				typeObj.typeStatus = true;
			}
			typeObj.typeValue += typeObj.typeArray[typeObj.typeArrayIndex].charAt(
				typeObj.charIndex
			);
			typeObj.charIndex += 1;
			setTimeout(typeText, typeObj.typingSpeed);
		} else {
			typeObj.typeStatus = false;
			setTimeout(eraseText, typeObj.newTextDelay);
		}
	}

	function eraseText() {
		if (typeObj.charIndex > 0) {
			if (!typeObj.typeStatus) {
				typeObj.typeStatus = true;
			}
			typeObj.typeValue = typeObj.typeArray[typeObj.typeArrayIndex].substring(
				0,
				typeObj.charIndex - 1
			);
			typeObj.charIndex -= 1;
			setTimeout(eraseText, typeObj.erasingSpeed);
		} else {
			typeObj.typeStatus = false;
			typeObj.typeArrayIndex += 1;
			if (typeObj.typeArrayIndex >= typeObj.typeArray.length) {
				typeObj.typeArrayIndex = 0;
			}
			setTimeout(typeText, typeObj.typingSpeed + 1000);
		}
	}

	onMounted(() => {
		setTimeout(typeText, typeObj.newTextDelay + 200);
	});
</script>

<style>
	[data-theme="night"] .header {
		background-image: linear-gradient(to top right, #0c496c, #46314b);
		background-position: center;
		background-repeat: no-repeat;
		background-size: cover;
	}
	[data-theme="winter"] .header {
		background-image: linear-gradient(to bottom left, #8080ee, #f4d8bb);
		background-position: center;
		background-repeat: no-repeat;
		background-size: cover;
	}

	div.cursor {
		display: inline-block;
		width: 3px;
		height: 24px;
		animation: cursorBlink 0.5s infinite;
	}
	@keyframes cursorBlink {
		49% {
			background-color: var(--tw-prose-headings);
		}
		50% {
			background-color: transparent;
		}
		99% {
			background-color: transparent;
		}
	}
	div.cursor.typing {
		animation: none;
	}
</style>

<template>
	<section class="header h-screen min-h-fit" id="header">
		<div class="grid grid-row-1 mx-auto h-screen content-end">
			<div class="container mx-auto px-[8vw] pb-[8vh]">
				<h1 class="m-0 py-5 font-sans font-bold antialiased drop-shadow-2xl">
					{{ typeObj.typeValue }}
					<div class="cursor" :class="{ typing: typeObj.typeStatus }"></div>
				</h1>
				<h2 class="m-0 py-5 font-sans font-bold antialiased drop-shadow-2xl">
					Jiaxing's Website
				</h2>
				<h3 class="m-0 py-5 drop-shadow-2xl italic">
					Aspiring Product Manager and Frontend Engineer
				</h3>
				<p class="m-0 font-medium py-5 antialiased drop-shadow-2xl">
					Built with
					<svg
						viewBox="0 0 221 65"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
						class="h-7 inline-block p-1"
					>
						<g clip-path="url(#a)">
							<path
								fill="currentColor"
								d="M82.5623 18.5705h7.3017l15.474 24.7415V18.5705h6.741v35.0576h-7.252L89.3025 28.938v24.6901h-6.7402V18.5705ZM142.207 53.628h-6.282v-3.916c-1.429 2.7559-4.339 4.3076-8.015 4.3076-5.822 0-9.603-4.1069-9.603-10.0175V28.3847h6.282v14.3251c0 3.4558 2.146 5.8592 5.362 5.8592 3.524 0 5.974-2.7044 5.974-6.4099V28.3847h6.282V53.628ZM164.064 53.2289l-6.026-8.4144-6.027 8.4144h-6.69l9.296-13.1723-8.58-12.0709h6.843l5.158 7.2641 5.106-7.2641h6.895l-8.632 12.0709 9.295 13.1723h-6.638ZM183.469 20.7726v7.6116h7.149v5.1593h-7.149v12.5311c0 .4208.17.8245.473 1.1223.303.2978.715.4654 1.144.4661h5.532v5.9547h-4.137c-5.617 0-9.293-3.2062-9.293-8.8109V33.5484h-5.056v-5.1642h3.172c1.479 0 2.34-.8639 2.34-2.2932v-5.3184h5.825Z"
							></path>
							<path
								fill-rule="evenodd"
								clip-rule="evenodd"
								d="M30.1185 11.5456c-1.8853-3.24168-6.5987-3.24169-8.484 0L1.08737 46.8747c-1.885324 3.2417.47133 7.2938 4.24199 7.2938H21.3695c-1.6112-1.4081-2.2079-3.8441-.9886-5.9341l15.5615-26.675-5.8239-10.0138Z"
								fill="#80EEC0"
							></path>
							<path
								d="M43.1374 19.2952c1.5603-2.6523 5.461-2.6523 7.0212 0l17.0045 28.9057c1.5603 2.6522-.39 5.9676-3.5106 5.9676h-34.009c-3.1206 0-5.0709-3.3154-3.5106-5.9676l17.0045-28.9057ZM209.174 53.8005H198.483c0-1.8514.067-3.4526 0-6.0213h10.641c1.868 0 3.353.1001 4.354-.934 1-1.0341 1.501-2.3351 1.501-3.9029 0-1.8347-.667-3.2191-2.002-4.1532-1.301-.9674-2.985-1.4511-5.054-1.4511h-2.601v-5.2539h2.652c1.701 0 3.119-.4003 4.253-1.2009 1.134-.8006 1.701-1.9849 1.701-3.5527 0-1.301-.434-2.3351-1.301-3.1023-.834-.8007-2.001-1.201-3.503-1.201-1.634 0-2.918.4837-3.853 1.4511-.9.9674-1.401 2.1517-1.501 3.5527h-6.254c.133-3.2358 1.251-5.7877 3.352-7.6558 2.135-1.868 4.887-2.8021 8.256-2.8021 2.402 0 4.42.4337 6.055 1.301 1.668.834 2.919 1.9515 3.753 3.3525.867 1.4011 1.301 2.9523 1.301 4.6536 0 1.9681-.551 3.636-1.651 5.0037-1.068 1.3344-2.402 2.235-4.004 2.7021 1.969.4003 3.57 1.3677 4.804 2.9022 1.234 1.5011 1.852 3.4025 1.852 5.7043 0 1.9347-.468 3.7028-1.402 5.304-.934 1.6012-2.301 2.8855-4.103 3.8529-1.768.9674-3.953 1.4511-6.555 1.4511Z"
								fill="#00DC82"
							></path>
						</g>
						<defs>
							<clipPath id="a">
								<path fill="#fff" d="M0 0h221v65H0z"></path>
							</clipPath>
						</defs>
					</svg>
					and
					<svg viewBox="0 0 248 31" class="w-auto h-5 inline-block p-1">
						<path
							fill-rule="evenodd"
							clip-rule="evenodd"
							d="M25.517 0C18.712 0 14.46 3.382 12.758 10.146c2.552-3.382 5.529-4.65 8.931-3.805 1.941.482 3.329 1.882 4.864 3.432 2.502 2.524 5.398 5.445 11.722 5.445 6.804 0 11.057-3.382 12.758-10.145-2.551 3.382-5.528 4.65-8.93 3.804-1.942-.482-3.33-1.882-4.865-3.431C34.736 2.92 31.841 0 25.517 0zM12.758 15.218C5.954 15.218 1.701 18.6 0 25.364c2.552-3.382 5.529-4.65 8.93-3.805 1.942.482 3.33 1.882 4.865 3.432 2.502 2.524 5.397 5.445 11.722 5.445 6.804 0 11.057-3.381 12.758-10.145-2.552 3.382-5.529 4.65-8.931 3.805-1.941-.483-3.329-1.883-4.864-3.432-2.502-2.524-5.398-5.446-11.722-5.446z"
							fill="#38bdf8"
						></path>
						<path
							fill-rule="evenodd"
							clip-rule="evenodd"
							d="M76.546 12.825h-4.453v8.567c0 2.285 1.508 2.249 4.453 2.106v3.463c-5.962.714-8.332-.928-8.332-5.569v-8.567H64.91V9.112h3.304V4.318l3.879-1.143v5.937h4.453v3.713zM93.52 9.112h3.878v17.849h-3.878v-2.57c-1.365 1.891-3.484 3.034-6.285 3.034-4.884 0-8.942-4.105-8.942-9.389 0-5.318 4.058-9.388 8.942-9.388 2.801 0 4.92 1.142 6.285 2.999V9.112zm-5.674 14.636c3.232 0 5.674-2.392 5.674-5.712s-2.442-5.711-5.674-5.711-5.674 2.392-5.674 5.711c0 3.32 2.442 5.712 5.674 5.712zm16.016-17.313c-1.364 0-2.477-1.142-2.477-2.463a2.475 2.475 0 012.477-2.463 2.475 2.475 0 012.478 2.463c0 1.32-1.113 2.463-2.478 2.463zm-1.939 20.526V9.112h3.879v17.849h-3.879zm8.368 0V.9h3.878v26.06h-3.878zm29.053-17.849h4.094l-5.638 17.849h-3.807l-3.735-12.03-3.771 12.03h-3.806l-5.639-17.849h4.094l3.484 12.315 3.771-12.315h3.699l3.734 12.315 3.52-12.315zm8.906-2.677c-1.365 0-2.478-1.142-2.478-2.463a2.475 2.475 0 012.478-2.463 2.475 2.475 0 012.478 2.463c0 1.32-1.113 2.463-2.478 2.463zm-1.939 20.526V9.112h3.878v17.849h-3.878zm17.812-18.313c4.022 0 6.895 2.713 6.895 7.354V26.96h-3.878V16.394c0-2.713-1.58-4.14-4.022-4.14-2.55 0-4.561 1.499-4.561 5.14v9.567h-3.879V9.112h3.879v2.285c1.185-1.856 3.124-2.749 5.566-2.749zm25.282-6.675h3.879V26.96h-3.879v-2.57c-1.364 1.892-3.483 3.034-6.284 3.034-4.884 0-8.942-4.105-8.942-9.389 0-5.318 4.058-9.388 8.942-9.388 2.801 0 4.92 1.142 6.284 2.999V1.973zm-5.674 21.775c3.232 0 5.674-2.392 5.674-5.712s-2.442-5.711-5.674-5.711-5.674 2.392-5.674 5.711c0 3.32 2.442 5.712 5.674 5.712zm22.553 3.677c-5.423 0-9.481-4.105-9.481-9.389 0-5.318 4.058-9.388 9.481-9.388 3.519 0 6.572 1.82 8.008 4.605l-3.34 1.928c-.79-1.678-2.549-2.749-4.704-2.749-3.16 0-5.566 2.392-5.566 5.604 0 3.213 2.406 5.605 5.566 5.605 2.155 0 3.914-1.107 4.776-2.749l3.34 1.892c-1.508 2.82-4.561 4.64-8.08 4.64zm14.472-13.387c0 3.249 9.661 1.285 9.661 7.89 0 3.57-3.125 5.497-7.003 5.497-3.591 0-6.177-1.607-7.326-4.177l3.34-1.927c.574 1.606 2.011 2.57 3.986 2.57 1.724 0 3.052-.571 3.052-2 0-3.176-9.66-1.391-9.66-7.781 0-3.356 2.909-5.462 6.572-5.462 2.945 0 5.387 1.357 6.644 3.713l-3.268 1.82c-.647-1.392-1.904-2.035-3.376-2.035-1.401 0-2.622.607-2.622 1.892zm16.556 0c0 3.249 9.66 1.285 9.66 7.89 0 3.57-3.124 5.497-7.003 5.497-3.591 0-6.176-1.607-7.326-4.177l3.34-1.927c.575 1.606 2.011 2.57 3.986 2.57 1.724 0 3.053-.571 3.053-2 0-3.176-9.66-1.391-9.66-7.781 0-3.356 2.908-5.462 6.572-5.462 2.944 0 5.386 1.357 6.643 3.713l-3.268 1.82c-.646-1.392-1.903-2.035-3.375-2.035-1.401 0-2.622.607-2.622 1.892z"
							fill="currentColor"
						></path>
					</svg>
				</p>
			</div>
		</div>
	</section>
</template>
